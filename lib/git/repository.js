...
Repository.prototype.newlog = function(sha, options, callback) {
  this.already_searched = {}
  return newwalk_log(this, sha, options);
}
...
var newwalk_log = function(repo, starting_sha, options, total_size) {
  // Empty array
  var array = [];
  var commits_to_visit= [starting_sha]; 
  var o = null, commit_sha = null, c = null, output = null, sha = null;
  options = options || {};
  var max_count= options["max_count"] || -1;

  while( commits_to_visit.length > 0 && (max_count == -1 ||  array.length< max_count) ) {
    sha= commits_to_visit.pop();
    if(repo.already_searched[sha]) continue;
    ...
    if( add_sha ) {
      //TODO: Re-Implement path limiter
      output = c.raw_log(sha);
      array.push([sha, output, c.committer.date]); 

      // Follow all parents unless --first-parent is specified
      var parent_count= c.parent.length-1;
      if( options["first_parent"] && parent_count >0) parent_count =0;
      for(var i = parent_count; i >= 0; i--) {
        var psha = c.parent[i];
        commits_to_visit.push(c.parent[i]);
      }
    }
  } 
  return array;
